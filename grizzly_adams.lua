...
local function greetCallback(npc, creature)
	...
	if player:getStorageValue(Storage.Quest.U8_5.KillingInTheNameOf.QuestLogEntry) ~= 0 then
		npcHandler:setMessage(MESSAGE_GREET, "Hi there, do you want to join the 'Paw and Fur - Hunting Elite'?")
	elseif
...
local messageBoss = {
	...
	{ "Well done, old chap. Now i shall assign you a special task. Rumour has it that there is an old skeleton somewhere in the jungle. Find Deathbine's hideout and kill it! Good luck, old chap ...", "Ask me about new {tasks} if you're up for further hunting mission. Be aware that you can only have one 'Deathbine' task active at the same time!" },
...
local messageBossStart = {
	...
	"Find Deathbine, the old skeleton, somewhere in the jungle. Find its hideout and kill it!",
...
local tier = {
	{
		allName = { "crocodiles", "trolls", "tarantulas", "skeletons", "minotaurs", "mammoths", "gnarlhounds", "terramites", "apes", "thornback tortoises", "gargoyles", "crocodile", "troll", "tarantula", "skeleton", "minotaur", "mammoth", "gnarlhound", "terramite", "ape", "thornback tortoise", "gargoyle" },
		withsName = { "crocodiles", "trolls", "tarantulas", "skeletons", "minotaurs", "mammoths", "gnarlhounds", "terramites", "apes", "thornback tortoises", "gargoyles" },
	},
	{
...
local messageStartTask = {
	["crocodiles"] = "They are a nuisance! You'll find them here in the jungle near the river. Hunt 3 crocodiles and you'll get a nice reward. Interested?",
	["trolls"] = "You'll find them in the under Kahralias. Hunt 3 trolls and you'll get a reward. Interested?",
	["tarantulas"] = "There is a veritable plague of tarantulas living in the caves north of the river to the east. Can you squish 3 tarantulas for the Hunting Elite. What do you say?",
	["minotaurs"] = "They can be found all over Tibia. You'll find them in mountain caves or rocky dungeons. Hunt 2 minotaurs of them and come back to me. Understood?",
...
local messageStartTaskAlt = {
	...
	["troll"] = messageStartTask["trolls"],
	["tarantula"] = messageStartTask["tarantulas"],
	["minotaur"] = messageStartTask["minotaurs"],
            ...
            if tasks.GrizzlyAdams[m].bossName:lower() == message:lower() then
                for n = 1, #tasks.GrizzlyAdams[m].rewards do
                    if table.contains({ REWARD_STORAGE, "storage", "stor" }, tasks.GrizzlyAdams[m].rewards[n].type:lower()) then
                        -- Get the task completion count
                        local taskCompletionCount = player:getStorageValue(REPEATSTORAGE_BASE + m)
                        if taskCompletionCount < 0 then taskCompletionCount = 0 end
                        
                        -- Get the boss kill count
                        local bossKillCount = player:getStorageValue(tasks.GrizzlyAdams[m].rewards[n].value[2])
                        if bossKillCount < 0 then bossKillCount = 0 end
                        
                        -- Check if player can fight the boss
                        if taskCompletionCount <= bossKillCount then
                            npcHandler:say("You need to complete the " .. tasks.GrizzlyAdams[m].raceName .. " task before you can fight " .. tasks.GrizzlyAdams[m].bossName .. " again.", npc, player)
                            return true
                        end
                        
                        -- Check if player has reached the maximum number of boss kills (3)
                        if bossKillCount >= 3 then
                            npcHandler:say("You have already defeated " .. tasks.GrizzlyAdams[m].bossName .. " the maximum number of times.", npc, player)
                            return true
                        end
...
local function creatureSayCallback(npc, creature, type, message)
		...
		if started and #started > 0 then
			...
			for i = 1, #started do
				...
				if player:getStorageValue(KillCounter + id) >= tasks.GrizzlyAdams[id].killsRequired then
					...
					-- Check if this task has a boss
					local hasBoss = false
					local bossStorage = nil
					
					if tasks.GrizzlyAdams[id].bossName then
						hasBoss = true
						-- Find the boss storage
						for j = 1, #tasks.GrizzlyAdams[id].rewards do
							if tasks.GrizzlyAdams[id].rewards[j].type == "storage" and tasks.GrizzlyAdams[id].rewards[j].value and #tasks.GrizzlyAdams[id].rewards[j].value >= 2 then
								bossStorage = tasks.GrizzlyAdams[id].rewards[j].value[2]
								break
							end
						end
					end
					
					-- Get task completion count and boss kill count
					local taskCompletionCount = player:getStorageValue(REPEATSTORAGE_BASE + id)
					if taskCompletionCount < 0 then taskCompletionCount = 0 end
					
					local bossKillCount = 0
					if bossStorage then
						bossKillCount = player:getStorageValue(bossStorage)
						if bossKillCount < 0 then bossKillCount = 0 end
					end
					...
					for j = 1, #tasks.GrizzlyAdams[id].rewards do
						...
						elseif table.contains({ REWARD_STORAGE, "storage", "stor" }, reward.type:lower()) and not deny then
							...
							elseif table.contains({ 1, 2 }, player:getStorageValue(reward.value[1])) then
								-- Only give boss points if the player has killed the boss for all previous task completions
								if not hasBoss or taskCompletionCount <= bossKillCount then
									player:setStorageValue(Storage.Quest.U8_5.KillingInTheNameOf.BossPoints, player:getStorageValue(Storage.Quest.U8_5.KillingInTheNameOf.BossPoints) + 1)
								end
							else
						...
						elseif table.contains({ REWARD_POINT, "points", "point" }, reward.type:lower()) and not deny then
							-- Only give points if the player has killed the boss for all previous task completions
							if not hasBoss or taskCompletionCount <= bossKillCount then
								local ratePoints = 1
								...
								-- If the player hasn't killed the boss, don't give points
								messageAltPoints = true
							end
		...
		if player:getLevel() < 50 then
			if player:getStorageValue(POINTSSTORAGE) >= 40 then
				npcHandler:say({
					...
					"You can try {crocodiles}, {trolls}, {tarantulas}, {skeletons}, {minotaurs}, {mammoths}, {gnarlhounds}, ...",
					"as well as {terramites}, {apes}, {thornback tortoises} and {gargoyles}.",
				}, npc, creature)
			else
				npcHandler:say("Alright, what would you like to hunt? {crocodiles}, {trolls}, {tarantulas}, {skeletons}, {minotaurs}, {mammoths}, {gnarlhounds}, {terramites}, {apes}, {thornback tortoises} or {gargoyles}.", npc, creature)
			end
	...
	elseif message ~= "" and player:canStartTask(message) then
		...
		-- Check if the task has a boss and if the player needs to kill it before starting the task again
		if tasks.GrizzlyAdams[task].bossName then
			-- Find the boss storage from the rewards
			local bossStorage = nil
			for _, reward in ipairs(tasks.GrizzlyAdams[task].rewards) do
				if reward.type == "storage" and reward.value and #reward.value >= 2 then
					bossStorage = reward.value[2] -- This should be the boss kill count storage
					break
				end
			end
			...
			if bossStorage then
				-- Get the task completion count
				local taskCompletionCount = player:getStorageValue(REPEATSTORAGE_BASE + task)
				if taskCompletionCount < 0 then taskCompletionCount = 0 end
				
				-- Get the boss kill count
				local bossKillCount = player:getStorageValue(bossStorage)
				if bossKillCount < 0 then bossKillCount = 0 end
				
				-- Check if player can start the task
				if taskCompletionCount > bossKillCount then
					npcHandler:say("You need to defeat " .. tasks.GrizzlyAdams[task].bossName .. " before you can take this task again.", npc, creature)
					return true
				end
				
				-- Check if player has reached the maximum number of completions (3)
				if taskCompletionCount >= 3 then
					npcHandler:say("You have already completed this task the maximum number of times.", npc, creature)
					return true
				end
			end
		end
		...
		if table.contains(tier[1].allName, message:lower()) then
			...
			elseif table.contains({ "skeletons", "skeleton" }, message:lower()) then
				local chanceX = math.random(2)
				local messageSkeletons = {
					[1] = "Interesting kind and not so easy to find. The fun begins when you want to hunt {Tiquanda's Revenge}. It's strong and smart like no other skeleton.",
					[2] = "Damn walking weed-thingies! You'll find them deeper in the jungle. Weed out 5 skeletons for our society. Alright?",
				}
				npcHandler:say(messageSkeletons[chanceX], npc, creature)
	...
	elseif message:lower() == "yes" and npcHandler:getTopic(playerId) == 1 then
		local taskId = choose[playerId]
		
		-- Check if taskId is valid
		if not taskId or not tasks.GrizzlyAdams[taskId] then
			npcHandler:say("I'm sorry, there seems to be an issue with this task. Please try again later.", npc, creature)
			return true
		end
		
		local taskData = tasks.GrizzlyAdams[taskId]
		
		-- Check if the task has a boss and if the player needs to kill it before starting the task again
		if taskData.bossName then
			-- Find the boss storage from the rewards
			local bossStorage = nil
			for _, reward in ipairs(taskData.rewards) do
				if reward.type == "storage" and reward.value and #reward.value >= 2 then
					bossStorage = reward.value[2] -- This should be the boss kill count storage
					break
				end
			...
			if bossStorage then
				-- Get the task completion count
				local taskCompletionCount = player:getStorageValue(REPEATSTORAGE_BASE + taskId)
				if taskCompletionCount < 0 then taskCompletionCount = 0 end
				...
				-- Check if player can start the task
				if taskCompletionCount > bossKillCount then
					npcHandler:say("You need to defeat " .. taskData.bossName .. " before you can take this task again.", npc, creature)
					return true
				end
		...
		-- If we get here, the player can start the task
		player:setStorageValue(QUESTSTORAGE_BASE + taskId, 1)
		player:setStorageValue(KillCounter + taskId, 0)
		
		-- Reset specific monster counter based on task
		if taskData.raceName then
			local taskName = taskData.raceName:lower()
			
			if taskName == "crocodiles" then
				player:setStorageValue(Storage.Quest.U8_5.KillingInTheNameOf.MonsterKillCount.CrocodileCount, 0)
			elseif taskName == "trolls" then
				player:setStorageValue(Storage.Quest.U8_5.KillingInTheNameOf.MonsterKillCount.TrollCount, 0)
			elseif taskName == "tarantulas" then
				player:setStorageValue(Storage.Quest.U8_5.KillingInTheNameOf.MonsterKillCount.TarantulaCount, 0)
			elseif taskName == "skeletons" then
				player:setStorageValue(Storage.Quest.U8_5.KillingInTheNameOf.MonsterKillCount.SkeletonCount, 0)
			elseif taskName == "minotaurs" then
				player:setStorageValue(Storage.Quest.U8_5.KillingInTheNameOf.MonsterKillCount.MinotaurCount, 0)
			end
		end
		
		-- Handle multi-creature tasks
		if taskData.creatures and #taskData.creatures > 1 then
			if taskData.raceName == "Apes" then
			...
			elseif taskData.raceName == "Quara Scouts" then
			...
			elseif taskData.raceName == "Underwater Quara" then
			...
			elseif taskData.raceName == "Nightmares" then
			...
			elseif taskData.raceName == "High Class Lizards" then
			...
			elseif taskData.raceName == "Sea Serpents" then
			...
			elseif taskData.raceName == "Drakens" then
		...
		-- Update kill storage
		if player:getStorageValue(KILLSSTORAGE_BASE + taskId) == 1 then
			player:setStorageValue(KILLSSTORAGE_BASE + taskId, player:getStorageValue(KILLSSTORAGE_BASE + taskId) - 1)
		else
			player:setStorageValue(KILLSSTORAGE_BASE + taskId, player:getStorageValue(KILLSSTORAGE_BASE + taskId) + 1)
		end
	...
	elseif table.contains({ "boss", "bosses" }, message:lower()) then
		...
		if player:getStorageValue(Storage.Quest.U8_5.KillingInTheNameOf.BossPoints) > 0 then
			if player:getLevel() < 50 then
				npcHandler:say("You can choose between the {Snapper}, {Grandpa Troll}, {Hide}, {Boneless Skeleton} and the {Minotaur Berserker} .", npc, creature)
				npcHandler:setTopic(playerId, 4)
			elseif player:getLevel() >= 50 and player:getLevel() < 80 then
				npcHandler:say({
					"You can choose between the {Snapper}, {Grandpa Troll}, {Hide}, the {Boneless Skeleton}, {Minotaur Berserker}, {Fleshcrawler}, {Ribstride}, {Bloodweb} and {Esmeralda}",
				}, npc, creature)
				npcHandler:setTopic(playerId, 5)
			elseif player:getLevel() >= 80 and player:getLevel() < 130 then
				npcHandler:say({
					"You can choose between the {Snapper}, {Grandpa Troll}, {Hide}, the {Boneless Skeleton}, {Minotaur Berserker}, {Fleshcrawler}, {Ribstride}, {Bloodweb}, {Esmeralda}, ...",
	...
	elseif table.contains({ "snapper", "hide", "deathbine", "bloodtusk" }, message:lower()) and npcHandler:getTopic(playerId) >= 4 and npcHandler:getTopic(playerId) <= 7 then
		-- Find the task associated with this boss
		local bossTaskId = nil
		local bossName = message:lower()
		
		for id, task in pairs(tasks.GrizzlyAdams) do
			if task and task.bossName and task.bossName:lower() == bossName then
				bossTaskId = id
				break
			end
		end
		...
		if bossTaskId then
			local taskData = tasks.GrizzlyAdams[bossTaskId]
			
			-- Get the task completion count
			local taskCompletionCount = player:getStorageValue(REPEATSTORAGE_BASE + bossTaskId)
			if taskCompletionCount < 0 then taskCompletionCount = 0 end
			
			-- Get the boss storage from the rewards
			local bossStorage = nil
			...
			if bossStorage then
				...
				-- Check if player can fight the boss
				if taskCompletionCount <= bossKillCount then
					npcHandler:say("You need to complete the " .. taskData.raceName .. " task before you can fight " .. taskData.bossName .. " again.", npc, creature)
					return true
				end
				
				-- Check if player has reached the maximum number of boss kills (3)
				if bossKillCount >= 3 then
					npcHandler:say("You have already defeated " .. taskData.bossName .. " the maximum number of times.", npc, creature)
					return true
				end
			end
		end
